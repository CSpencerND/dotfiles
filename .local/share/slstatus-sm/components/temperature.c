...
const char *temp()
{
        ...
        FILE *fptr;
        char thermal_file[64];

        char home[64] = "/home/";
        char *user = getenv("USER");
        char *thermal = "/.cache/thermal_path";
        char *user_path = strcat(home, user);
        char *thermal_path = strcat(user_path, thermal);

        fptr = fopen(thermal_path, "r");
        if (fptr != NULL)
        {
            fgets(thermal_file, 38, fptr);
            fclose(fptr);
            printf("%s", thermal_file);
        }

        if (pscanf(thermal_file, "%ju", &temp) != 1)
        {
                return NULL;
        }
