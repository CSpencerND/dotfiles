...
struct Bar
{
        ...
        int w[BARRULES];    // width, array length == barrules, then use r index
                            // for lookup purposes
...
void applyrules(Client *c)
{
        ...
        for (i = 0; i < LENGTH(rules); i++)
        {
                ...
                if ((!r->title || strstr(c->name, r->title)) &&
                    ...
                    (!r->wintype ||
                     wintype == XInternAtom(dpy, r->wintype, False)))
                {
...
void drawbarwin(Bar *bar)
{
        ...
        int rx, lx, rw, lw; // bar size, split between left and right if a
                            // center module is added
        const BarRule *br;
        ...
        for (r = 0; r < LENGTH(barrules); r++)
        {
                ...
                if (lw <= 0)
                { // if left is exhausted then switch to right side, and vice
                  // versa
                        lw = rw;
                        lx = rx;
                }
...
void setup(void)
{
        ...
        /* the one line of bloat that would have saved a lot of time for a lot
         * of people */
...
void showhide(Client *c)
{
        ...
        if (ISVISIBLE(c))
        {
                ...
                if (selmon->sel == c)
                        ...
                        XMoveWindow(dpy, c->win, c->x + borderpx,
                                    c->y + borderpx);
...
void unfocus(Client *c, int setfocus, Client *nextfocus)
{
        ...
        if (c->isfloating)
                XSetWindowBorder(dpy, c->win,
                                 scheme[SchemeNorm][ColFloat].pixel);
        else
                XSetWindowBorder(dpy, c->win,
                ...
                for (bar = m->bar; bar; bar = bar->next)
                {
                        ...
                        if (!bar->win)
                        {
                                bar->win = XCreateWindow(
                                        dpy, root, bar->bx, bar->by, bar->bw,
                                        bar->bh, 0, DefaultDepth(dpy, screen),
                                        CopyFromParent,
                                        DefaultVisual(dpy, screen),
                                        CWOverrideRedirect | CWBackPixmap |
                                                CWEventMask,
                                        &wa);
                                XDefineCursor(dpy, bar->win,
